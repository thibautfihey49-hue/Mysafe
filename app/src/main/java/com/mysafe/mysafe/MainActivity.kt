...
private val REQUETE_PERMISSIONS = 12345    private val permissionsRequises = arrayOf(        Manifest.permission.ACCESS_FINE_LOCATION,        Manifest.permission.ACCESS_COARSE_LOCATION,        Manifest.permission.SEND_SMS,        Manifest.permission.RECEIVE_SMS,        Manifest.permission.POST_NOTIFICATIONS    )    private fun demanderPermissions() {        val manquantes = permissionsRequises.filter {            ContextCompat.checkSelfPermission(this, it) == PackageManager.PERMISSION_DENIED        }.toTypedArray()        if (manquantes.isNotEmpty()) {            ActivityCompat.requestPermissions(this, manquantes, REQUETE_PERMISSIONS)        }    }    override fun onRequestPermissionsResult(        requestCode: Int,        permissions: Array<out String>,        grantResults: IntArray    ) {        super.onRequestPermissionsResult(requestCode, permissions, grantResults)        if (requestCode == REQUETE_PERMISSIONS) {            val ok = grantResults.all { it == PackageManager.PERMISSION_GRANTED }            Toast.makeText(this, msg, Toast.LENGTH_SHORT).show()        }    }
}
